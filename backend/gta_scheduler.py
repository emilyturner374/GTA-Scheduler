...
def isTimeSlotAvailable(student, course, section):
    ...
    for day in courses[course]['sections'][section]['days']:
        if day+'-'+str(section) in students[student]['takenTimeSlots']:
            for timeSlot in students[student]['takenTimeSlots'][day+'-'+str(section)]:
            ...
            if courses[course]['sections'][section]['status'] == 'open': # check if the status of the section within the course is open
                for student in students: # if it's open, loop through each student
                    if isTimeSlotAvailable(student, course, section): # checks if the time slot of that section is available compared to the time slots already taken by the student
                        students[student]['coursesBeingTaught'].append(course+'-'+str(section)) # add the course to the list of courses that the student is teaching
                        courseTimeSlot = courses[course]['sections'][section]['timeSlot'] # stores the course's section's time slot

                        # appends the days and time slots of the course and its section to the student object
                        for day in courses[course]['sections'][section]['days']:
                            if day+'-'+str(section) in students[student]['takenTimeSlots']:
                                students[student]['takenTimeSlots'][day+'-'+str(section)].append(courseTimeSlot) # appends the section's time slot to the student's takenTimeSlots if the student is already teaching on that day
                            else:
                                students[student]['takenTimeSlots'][day+'-'+str(section)] = [courseTimeSlot] # creates new entry and begins the time interval list
